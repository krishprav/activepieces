...
const polling: Polling<any, any> = {
  ...
  items: async ({ auth, propsValue }) => {
    ...
    const resp = await emailOctopusRequest<{ data: ClickEvent[] }>({
      auth,
      method: HttpMethod.GET,
      path: `/campaigns/${campaignId}/events/clicks`,
...
export const eoEmailClicked = createTrigger({
  auth: emailOctopusAuth,
  name: 'emailoctopus_email_clicked',
